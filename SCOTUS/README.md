...
- Specify date range for cases you want in mp3_curl_commands.py (We use 2014 & 2015 line 116)
...
Simply run the `oyez2rttm.py` script to write the transcriptions to the `rttm` folder in RTTM format. This also generates and saves a dictionary of present speakers and identification numbers so that each speaker is labelled consistently throughout the experiments. If you are processing the oyez transcriptions for the BERT model you can use the `Oyez_Transcription_Processing.ipynb`. 